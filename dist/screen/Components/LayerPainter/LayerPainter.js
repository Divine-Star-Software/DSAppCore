...
import { AddClass, AddEvents, AddStyles, UseCascade, div, element, } from "elmtree";
...
export class LayerPainter {
    ...
    drawNode(x, y, addToMap = false) {
        ...
        for (let sx = x; sx < x + width; sx += 16) {
            for (let sy = y; sy < y + height; sy += 16) {
                if (sx != y && sy != y) {
                    if (this.nodes.get(space.chunk.getKeyXYZ(sx, sy, 0)))
                        continue;
                }
                if (addToMap)
                    this._visistedMap.add(sx, sy, 0);
    ...
    import(nodes) {
        nodes.forEach((_) => {
            this.nodes.set(space.chunk.getKeyXYZ(_.position[0], _.position[1], _.position[2]), _);
        });
        return this;
    }
    store() {
    ...
    render() {
        ...
        for (const nodeId in this.nodeData.palette) {
            ...
            paletteElms.push(element("div", [
                AddClass([
                    "layer-painter-node",
                    this.activeNode == nodeId ? "active" : "",
                ]),
                this.updatePalette.addToElment((elm) => {
                ...
                element("p", [AddClass(["layer-painter-node-name"])], `${node.name}`),
            ]));
        }
