...
import { AddAttributes, Component, ComponentUpdate } from "elmtree";
...
export const menuButton = (data: MenuData, sub = false): ElmTreeData =>
  div(
    `${sub ? "sub-" : ""}menu-button ${
      MENU_STATE.node == data[0] ? "active" : ""
    }`,
    paragraph(data[0]),
    [
      AddAttributes({
