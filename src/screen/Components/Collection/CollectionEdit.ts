...
import { ObjectCollectionData } from "@util/ObjectCollection.types";
import { Vec2Array, Vec3Array } from "divine-voxel-engine/Math";

//import { FileSystem } from "@system/File/FileSystem";
type CollectionEditType = {
  ...
  ) => FormBuilderElements<Record<string, any>> | false;
};
const vectorInput = (
...
export class CollectionEditor {
  customTypes = new Map<string, CollectionEditType>([
    [
      "file-path",
      {
        id: "file-path",
        getElements(node, editor) {
          if (node.input?.type != "file-path") return false;
          const value: string = editor.collection.get(node.id);
